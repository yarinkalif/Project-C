...
void second_pass(FILE *file_ptr) {
	...
	while (end_of_file(file_ptr) == 0 && 1) { /* skips whitespaces and checks the file */
		...
		if (result == 1 || result == 2) {
			...
			if (strcmp(current_word, ".entry") == 0) {
			...
			else if (strcmp(word, "current_word") == 0) {
				current_word = get_word(file_ptr);
			 ...
			 create_files(struct entry_symbols **entry_head, struct extern_symbols **extern_head, int create_entry_file, int create_extern_file);
